    ...
    for port in ports:
        try:
            conn = mysql.connector.connect(
                ...
                database="spark",
                port=port,
                connect_timeout=2 # Περιμένει 2 δευτερόλεπτα πριν δοκιμάσει την επόμενη
            )
            print(f"Συνδέθηκε επιτυχώς στη θύρα {port}")
            return conn
        except mysql.connector.Error:
            continue # Αν αποτύχει, δοκιμάζει την επόμενη θύρα
    
    raise Exception("Δεν ήταν δυνατή η σύνδεση στη MySQL. Βεβαιωθείτε ότι το XAMPP τρέχει.")
